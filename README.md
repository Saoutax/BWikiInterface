...
`src/gadgets/MyGadget/Gadget-MyGadget.css`：
...
A：确保 `src/` 目录结构正确，小工具主文件命名符合 `Gadget-[目录名].js` 或 `Gadget-[目录名].css` 格式。
...
- [Lightning CSS](https://lightningcss.dev/) - 极速的 CSS 解析器、转换器与压缩器
